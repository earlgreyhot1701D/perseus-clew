...
- Scan public URLs and public GitHub repos at MVP (frontend scanning)
- Scan OpenAPI specifications on the backend at MVP (powers the 50-site benchmark, not yet in the user-facing tool)
...
- Store anonymous scan results for 24 hours (TTL) to enable shareable result links, then auto-delete. No PII, no IP, no cross-scan linkage.
- Run its scan engine, simulation, and refresh on AWS; serve its Next.js frontend (including API routes) on Vercel
...
For frontend scans: the six check modules ported from Hermes Clew v1 (Python) to JavaScript (Node.js on Lambda). Each module examines HTML or JSX/TSX files, counts patterns, and returns a structured result with `passed`, `total`, and `findings` fields. *CHECK HERMES: this matches the exact contract used by every Python check module in Hermes v1, verified against `scan/check_semantic_html.py` and `scan/scoring.py`.*
...
The scoring module (also ported from Hermes) consumes check module outputs and applies the weights defined in `SCORING.md`. Scoring knows nothing about HTML or OpenAPI. It knows weights and math.
...
This layer exists because a score is a starting point. The narrative is what makes findings actionable. Hermes Clew v1 proved the deterministic-plus-LLM pattern works; Perseus extends it.
...
**Model:** Claude Haiku 4.5 (`claude-haiku-4-5-20251001`). Pricing is $1/M input, $5/M output on Bedrock. The model is active with no deprecation notice, and Anthropic commits to at least 60 days notice before any retirement. The specific model ID is stored in Lambda environment configuration, not hardcoded. Swapping to a different model is a one-line env change.

**Orchestration at MVP:** The simulation runs synchronously within the scan Lambda. Scan Lambda completes Layer 1 checks, calls `simulation.run(findings, parsedContent)` as a function call against the module, waits for Bedrock, returns everything in one response. This keeps infrastructure simple.
...
```
                         ...
                         +------------------------+
                            |                  |
              scan request  |                  | benchmark reads
              (to AWS)      v                  | (DynamoDB, direct)
                         +------------------------+      |
            ...
            |  Logs/Metrics  | |  (hero line +  | |  - Benchmark   |
            |  Counters      | |   simulation)  | |  - ScanCache   |
            |                | |                | |    (15m TTL)   |
            |                | |                | |  - ScanResults |
            |                | |                | |    (24h TTL)   |
            +----------------+ +----------------+ +----------------+
...
Note on topology: the Vercel API routes are load-bearing, not a CDN relabel. `/api/benchmark` reads DynamoDB directly for benchmark display. `/api/scan` initiates a scan against the AWS scan Lambda (via API Gateway). The scan engine, Bedrock, and the result store stay on AWS; the frontend and its API routes are on Vercel. DynamoDB is the shared store both sides touch.
...
### Side flow: Layer 2 simulation (called by scan Lambda when enabled)
...
```
  ...
  linked to Layer 1 findings
           |
           v
  Returned to Scan Lambda,
  included in report
```

The simulation runs synchronously within the scan Lambda at MVP (single round-trip to Bedrock before returning the report). The diagram shows it as a conceptually separate box because the module is cleanly isolated, but it shares the Lambda process with Layer 1 at MVP. See Section 2 (Layer 2) for the upgrade path to async invocation if needed.
...
|-----------------|-----------------------------------|------------------------------------------------------------|
| Frontend        | Next.js (App Router)              | Component-based UI for growing dashboard. API routes do real work: scan initiation and direct DynamoDB benchmark reads. Self-scanning requirement means semantic HTML throughout (SSR/SSG output is agent-readable). |
| Hosting         | Vercel                            | Frontend + API routes. H0 requirement and a good fit. API routes are load-bearing (not a CDN relabel). v2 change: supersedes the v1 CloudFront + S3 decision for the frontend. Scan engine and refresh stay on AWS. |
...
| Serverless      | AWS Lambda                        | Per-scan pricing. No idle compute cost. Cold-start is acceptable for a scan that runs in under 30 seconds. |
...
| AI (Layer 2)    | AWS Bedrock + Claude Haiku 4.5    | Model ID: `claude-haiku-4-5-20251001`. Pricing: $1/M input, $5/M output. Active, no deprecation. Model ID in env config for easy swap. |
...
| Database        | DynamoDB                          | Benchmark data (read-heavy), ephemeral scan results (24h TTL), and the signed-in user partition. Simple access patterns. *OPEN QUESTION from checklist: whether percentile and trend queries will fit DynamoDB or require RDS. Spike needed before benchmark queries get complex.* |
| Monitoring      | CloudWatch                        | Lambda logs automatically. Metrics and Alarms built in. Credits cover it. No separate tool needed. |
...
`ScanCache` is deliberately separate from `ScanResults` (Table 3, Table 4 in the data model). The cache exists to dedup target-site fetches: short-lived, keyed by URL hash, disposable. The result store exists to back shareable links: 24-hour life, keyed by an opaque result id. They have different keys, different lifetimes, and different purposes, so they are different tables. A scan writes to both.
...
Each module returns `{ passed, total, findings }`. The modules run sequentially, not in parallel, because the parsed HTML is shared and the marginal cost of each module is low. Sequential execution also keeps logs readable. *CHECK HERMES: the Python scanner runs checks sequentially in a dict comprehension; we replicate that contract.*
...
The scan event is logged as a structured JSON entry in CloudWatch. Domain only, never full URLs. No PII. No HTML. No raw IPs. Counters increment in CloudWatch Metrics.

The report is returned to the frontend. After the response is sent, the scan result is written to DynamoDB asynchronously with a 24-hour TTL (anonymous: domain, scores, findings, timestamp; no PII, no IP, no cross-scan linkage). The write is fail-soft: if it fails, the user still has their report, and the failure is logged to CloudWatch, never surfaced. This row is what a shareable result link resolves against; after 24 hours the TTL deletes it and the link resolves to an expired state. If the user is signed in, the result is also written to their account partition (a small honest signal is shown if that history write fails; anonymous scans get no such signal because there is nothing to save to).
...
Aggregate counters for the measurement and privacy section of the product review. Incremented atomically. No user identifiers.
...
**Table 3: ScanResults (24h TTL, anonymous)**

Stores a scan result long enough to power a shareable result link, then auto-deletes. Written async and fail-soft by the scan Lambda after the response is sent.
...
**Table 5: ScanCache (15-min TTL, dedup only)**

Exists solely to avoid re-fetching a target site when the same URL is scanned repeatedly in a short window. Keyed by a hash of the URL, never the full URL. Separate from ScanResults by design: different key (URL hash vs opaque result id), different lifetime (15 min vs 24h), different purpose (good-citizen dedup vs shareable links). A scan writes to both ScanCache and ScanResults.
...
Anonymous scan results ARE stored for 24 hours (ScanResults, TTL-deleted) to enable shareable links. Signed-in users' scans are account-linked in the Users partition. Neither holds full content, IPs, or cross-anonymous-scan linkage.

### Paid tier extensions (STUB, not built at MVP)

The Users table (Table 4) exists at MVP as a stub: auth wired, schema present, empty-state history. The features that read it heavily are paid tier: trend tracking, score-over-time, delta tracking, scheduled rescan configs, team/multi-domain. These are deferred. The data policy is published at launch (anonymous 24h storage and the signed-in stub both exist), not just before paid tier.

OPEN QUESTION from checklist: DynamoDB may fight against percentile and trend queries as the benchmark grows. A spike is needed before the benchmark gets complex. If DynamoDB cannot support those query patterns, the architecture switches to RDS (Postgres) for benchmark data while keeping ScanCounters in DynamoDB. This decision is deferred, not blocking MVP.
...
- **Frontend topology:** Next.js on Vercel with load-bearing API routes (`/api/scan` initiates against AWS, `/api/benchmark` reads DynamoDB directly). Supersedes v1 CloudFront + S3. Scan engine, Bedrock, refresh stay on AWS.
...
1. **Bedrock model:** Claude Haiku 4.5 (`claude-haiku-4-5-20251001`). Active, no deprecation. Model ID in env config for easy swap.
...
8. **Check execution:** Sequential (matches Hermes v1).
...
- Hermes v1 check module contract (verified against `scan/scanner.py` and `scan/check_semantic_html.py`)
- NEVER list (reproduced from product review decisions)
- Data flow steps 1-10 (composed from product review scan prerequisites and Hermes scanner pattern)
- Measurement metrics (reproduced from product review measurement section)
- DynamoDB tables for BenchmarkScans and ScanCounters (composed from product review, single-table concepts, and measurement requirements)
...
- **Hermes rating language vs the new three-band scheme.** The three bands are locked above. Specific thresholds (what score qualifies as Agent-Ready vs Partially Ready vs Not Yet Readable) are defined in `SCORING.md`, not here.
